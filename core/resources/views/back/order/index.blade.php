    ...
    <div class="card mb-4">
        <div class="card-body">
            <div class="d-sm-flex align-items-center justify-content-between">
                <h3 class=" mb-0 bc-title"><b>{{request()->input('type') == 'Pending' ? __('New') : (request()->input('type') == 'In Progress' ? __('Delivery in Progress') : (request()->input('type') == 'Send to Delivery House' ? __('Send to Delivery House') : (request()->input('type') == 'Accepted' ? __('Accepted') : (request()->input('type') ? request()->input('type') : __('All')))))}} {{ __('Orders') }}</b></h3>
                <div class="right">
    ...
    <div class="row mb-3">
        <div class="col-12">
            <div class="btn-group flex-wrap" role="group">
                <a href="{{ route('back.order.index') }}" class="btn btn-sm {{ !request('type') ? 'btn-primary' : 'btn-outline-primary' }}">
                    {{ __('All Orders') }}
                </a>
                <a href="{{ route('back.order.index', ['type' => 'Pending']) }}" class="btn btn-sm {{ request('type') == 'Pending' ? 'btn-warning text-dark font-weight-bold' : 'btn-outline-warning text-dark' }}">
                    <i class="fas fa-clock mr-1"></i> {{ __('New Orders') }}
                </a>
                <a href="{{ route('back.order.index', ['type' => 'Accepted']) }}" class="btn btn-sm {{ request('type') == 'Accepted' ? 'btn-primary' : 'btn-outline-primary' }}">
                    <i class="fas fa-check mr-1"></i> {{ __('Accepted') }}
                </a>
                <a href="{{ route('back.order.index', ['type' => 'Send to Delivery House']) }}" class="btn btn-sm {{ request('type') == 'Send to Delivery House' ? 'text-white' : '' }}" style="{{ request('type') == 'Send to Delivery House' ? 'background-color: #6f42c1; border-color: #6f42c1;' : 'border-color: #6f42c1; color: #6f42c1;' }}">
                    <i class="fas fa-warehouse mr-1"></i> {{ __('Send to Delivery House') }}
                </a>
                <a href="{{ route('back.order.index', ['type' => 'In Progress']) }}" class="btn btn-sm {{ request('type') == 'In Progress' ? 'btn-info' : 'btn-outline-info' }}">
                    <i class="fas fa-truck mr-1"></i> {{ __('Delivery in Progress') }}
                </a>
                <a href="{{ route('back.order.index', ['type' => 'Delivered']) }}" class="btn btn-sm {{ request('type') == 'Delivered' ? 'btn-success' : 'btn-outline-success' }}">
                    <i class="fas fa-check-circle mr-1"></i> {{ __('Delivered Orders') }}
                </a>
                <a href="{{ route('back.order.index', ['type' => 'Canceled']) }}" class="btn btn-sm {{ request('type') == 'Canceled' ? 'btn-danger' : 'btn-outline-danger' }}">
