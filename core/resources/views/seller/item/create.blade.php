...
<div class="container-fluid">
    ...
    <div class="card mb-4">
        <div class="card-body">
            <div class="d-sm-flex align-items-center justify-content-between">
                <h3 class="mb-0 text-dark"><b><i class="fas fa-plus-circle text-primary mr-2"></i> {{ __('Create New Product') }}</b></h3>
    ...
    <form class="tab-form" action="{{ route('seller.item.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <input type="hidden" name="item_type" value="{{ request('type') == 'digital' ? 'digital' : 'normal' }}">
            ...
            <div class="col-lg-8">
                ...
                <div class="card shadow-sm mb-4">
                    <div class="card-header bg-light py-2">
                        <h6 class="m-0 font-weight-bold text-dark"><i class="fas fa-image text-primary mr-1"></i> {{ __('Product Images') }}</h6>
                    </div>
                    <div class="card-body">
                        <div class="form-group mb-4">
                            <label class="font-weight-bold">{{ __('Featured Image (Main Photo)') }} <span class="text-danger">*</span></label>
                            <div class="custom-file">
                                <input type="file" name="photo" id="photo" class="custom-file-input upload-photo" accept="image/*" required onchange="previewImg(this, 'featured_preview')">
                                <label class="custom-file-label" for="photo">{{ __('Choose Main Image (800x800 recommended)') }}</label>
                            </div>
                            <small class="text-info mt-1 d-block">{{ __('Image Size Should Be 800 x 800 or square size.') }}</small>
                            <div class="mt-2 text-center">
                                <img src="" id="featured_preview" class="img-fluid rounded d-none border p-1" style="max-height: 180px;">
                            </div>
                        </div>

                        <div class="form-group mb-0">
                            <label class="font-weight-bold">{{ __('Gallery Additional Images') }}</label>
                            <div class="custom-file">
                                <input type="file" name="galleries[]" id="galleries" class="custom-file-input" accept="image/*" multiple>
                                <label class="custom-file-label" for="galleries">{{ __('Choose Multiple Images') }}</label>
                            </div>
                            <small class="text-muted">{{ __('Hold Ctrl/Cmd to select multiple images for product gallery.') }}</small>
                        </div>
                    </div>
                </div>
                ...
                <div class="card shadow-sm mb-4">
                    ...
                    <div class="card-body">
                        <div class="form-group mb-3">
                            <label for="sort_details" class="font-weight-bold">{{ __('Short Description / Highlights') }} <span class="text-danger">*</span></label>
                            <textarea name="sort_details" id="sort_details" rows="3" class="form-control" placeholder="{{ __('Short summary shown next to price on product page') }}" required>{{ old('sort_details') }}</textarea>
                        </div>
                ...
                <div class="card shadow-sm mb-4">
                    ...
                    <div class="card-body">
                        <div class="form-group mb-3">
                            ...
                            <input type="text" name="tags" class="tags" id="tags" placeholder="{{ __('Tags (press comma or enter to separate)') }}" value="{{ old('tags') }}">
                ...
                <div class="card shadow-sm mb-4">
                    ...
                    <div class="card-body">
                        <div class="form-group mb-3">
                            ...
                            <input type="text" name="meta_keywords" class="tags" id="meta_keywords" placeholder="{{ __('Enter Meta Keywords') }}" value="{{ old('meta_keywords') }}">
                        ...
                        <div class="form-group mb-0">
                            ...
                            <textarea name="meta_description" id="meta_description" class="form-control" rows="4" placeholder="{{ __('Enter Meta Description for SEO') }}">{{ old('meta_description') }}</textarea>
                        </div>
                    </div>
                </div>

                <!-- Digital Product File (if digital) -->
                @if(request('type') == 'digital')
                    <div class="card shadow-sm mb-4">
                        <div class="card-header bg-light py-2">
                            <h6 class="m-0 font-weight-bold text-dark"><i class="fas fa-download mr-1"></i> {{ __('Digital Product Download File') }}</h6>
                        </div>
                        <div class="card-body">
                            <div class="form-group mb-0">
                                <label class="font-weight-bold">{{ __('Upload File (ZIP, PDF, MP3, etc.)') }} <span class="text-danger">*</span></label>
                ...
                <div class="card shadow-sm mb-4">
                    ...
                    <div class="card-body">
                        <div class="form-group mb-3">
                            ...
                            <input type="number" step="0.01" min="1" name="discount_price" id="discount_price" class="form-control font-weight-bold" placeholder="0.00" value="{{ old('discount_price') }}" required>
                        </div>

                        <div class="form-group mb-3">
                            <label for="previous_price" class="font-weight-bold">{{ __('Previous / Original Price') }} ({{ PriceHelper::adminCurrency() }})</label>
                            <input type="number" step="0.01" min="0" name="previous_price" id="previous_price" class="form-control" placeholder="0.00" value="{{ old('previous_price', 0) }}">
                    ...
                    <div class="card-body">
                        @if(request('type') != 'digital')
                            <div class="form-group mb-3">
                                ...
                                <input type="number" min="0" step="1" name="stock" id="stock" class="form-control font-weight-bold" placeholder="{{ __('Total in stock') }}" value="{{ old('stock', 10) }}" required>
                            </div>
                        @else
                            <input type="hidden" name="stock" id="stock" value="999999">
                        @endif

                        <input type="hidden" name="tax_id" value="{{ $taxes->where('status', 1)->first()->id ?? ($taxes->first()->id ?? '') }}">

                        <div class="form-group mb-3">
                            <label for="sku" class="font-weight-bold">{{ __('SKU / Product ID') }} <span class="text-danger">*</span></label>
                            <input type="text" name="sku" id="sku" class="form-control text-uppercase font-weight-bold" placeholder="{{ __('Enter SKU / Product ID') }}" value="{{ old('sku', \App\Repositories\Back\ItemRepository::generateAutoSku()) }}">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" name="video" id="video" class="form-control" placeholder="{{ __('e.g. YouTube video embed link') }}" value="{{ old('video') }}">
                        </div>
