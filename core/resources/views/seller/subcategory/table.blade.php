    ...
    <td class="text-center" style="vertical-align: middle;">
        <div class="dropdown">
            ...
            <div class="dropdown-menu animated--fade-in dropdown-menu-right" aria-labelledby="dropdownMenuButton_{{ $data->id }}">
              <a class="dropdown-item text-success font-weight-bold" href="{{ route('seller.subcategory.status', [$data->id, 1]) }}"><i class="fas fa-check mr-1"></i> {{ __('Enable') }}</a>
              <a class="dropdown-item text-danger font-weight-bold" href="{{ route('seller.subcategory.status', [$data->id, 0]) }}"><i class="fas fa-times mr-1"></i> {{ __('Disable') }}</a>
            </div>
        </div>
    </td>
    <td class="text-center" style="vertical-align: middle;">
        <div class="d-flex justify-content-center align-items-center" style="gap: 5px;">
            <a class="btn btn-primary btn-sm px-2 py-1" href="{{ route('seller.subcategory.edit', $data->id) }}" title="{{ __('Edit') }}">
                <i class="fas fa-edit"></i>
            </a>
            <a class="btn btn-danger btn-sm px-2 py-1" data-toggle="modal" data-target="#confirm-delete" href="javascript:;" data-href="{{ route('seller.subcategory.destroy', $data->id) }}" title="{{ __('Delete') }}">
