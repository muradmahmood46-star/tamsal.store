    ...
    <div class="card shadow"><div class="card-body">
        ...
        <form method="post" action="{{ $deal ? route('seller.deal.update', $deal->id) : route('seller.deal.store') }}">
            ...
            <div class="row"><div class="col-lg-7">
                ...
                <div class="alert alert-light border"><div>{{ __('Selected total:') }} <strong id="deal-total">PKR 0.00</strong></div><div>{{ __('Buyer save:') }} <strong class="text-danger" id="deal-savings">PKR 0.00</strong></div><div>{{ __('Final deal price:') }} <strong class="text-success" id="deal-final">PKR 0.00</strong></div></div>
...
function updateDealTotal() { var total = 0; document.querySelectorAll('.deal-product-checkbox:checked').forEach(function(input) { total += parseFloat(input.dataset.price) || 0; }); var discount = parseFloat(document.getElementById('discount_value').value) || 0; var saving = document.getElementById('discount_type').value === 'percent' ? total * discount / 100 : discount / dealCurrencyValue; saving = Math.min(saving, total); document.getElementById('deal-total').textContent = dealMoney(total); document.getElementById('deal-savings').textContent = dealMoney(saving); document.getElementById('deal-final').textContent = dealMoney(Math.max(0, total - saving)); }
document.querySelectorAll('.deal-product-checkbox,#discount_type,#discount_value').forEach(function(input) { input.addEventListener('change', updateDealTotal); input.addEventListener('input', updateDealTotal); }); document.getElementById('is_free_delivery').addEventListener('change', function() { document.getElementById('delivery-charge-group').style.display = this.checked ? 'none' : ''; document.getElementById('delivery_charge').disabled = this.checked; }); document.getElementById('is_free_delivery').dispatchEvent(new Event('change')); updateDealTotal();
</script>
@endsection
