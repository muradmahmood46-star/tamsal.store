    ...
    <div class="card shadow"><div class="card-body">
        ...
        <form method="post" action="{{ $deal ? route('seller.deal.update', $deal->id) : route('seller.deal.store') }}">
            ...
            <div class="row"><div class="col-lg-7">
                ...
                <div class="alert alert-light border"><div>{{ __('Selected total:') }} <strong id="deal-total">PKR 0.00</strong></div><div>{{ __('You save:') }} <strong class="text-danger" id="deal-savings">PKR 0.00</strong></div><div>{{ __('Final deal price:') }} <strong class="text-success" id="deal-final">PKR 0.00</strong></div></div>
...
document.querySelectorAll('.deal-product-checkbox,#discount_type,#discount_value').forEach(function(input) { input.addEventListener('change', updateDealTotal); input.addEventListener('input', updateDealTotal); }); updateDealTotal();
</script>
@endsection
