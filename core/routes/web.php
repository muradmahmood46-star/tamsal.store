...
Route::group(['middleware' => ['adminlocalize', 'demo']], function () {
    ...
    Route::prefix('admin')->group(function () { 
        ...
        //------------ STORE REQUESTS (VENDOR APPLICATIONS) ------------
        Route::get('store-requests', 'Back\StoreRequestController@index')->name('back.store_request.index');
        Route::post('store-requests/approve/{id}', 'Back\StoreRequestController@approve')->name('back.store_request.approve');
        Route::post('store-requests/reject/{id}', 'Back\StoreRequestController@reject')->name('back.store_request.reject');
        Route::post('store-requests/block/{id}', 'Back\StoreRequestController@blockSeller')->name('back.store_request.block');
        Route::post('store-requests/unblock/{id}', 'Back\StoreRequestController@unblockSeller')->name('back.store_request.unblock');
        Route::post('store-requests/send-message/{id}', 'Back\StoreRequestController@sendMessage')->name('back.store_request.send_message');
        Route::get('store-requests/messages/{id}', 'Back\StoreRequestController@fetchMessages')->name('back.store_request.messages');
        Route::delete('store-requests/delete/{id}', 'Back\StoreRequestController@delete')->name('back.store_request.delete');

        //------------ STORE UNBLOCK REQUESTS (APPEALS) ------------
        Route::get('unblock-requests', 'Back\UnblockRequestController@index')->name('back.unblock_request.index');
        Route::get('unblock-requests/chat/{id}', 'Back\UnblockRequestController@fetchChat')->name('back.unblock_request.chat');
        Route::post('unblock-requests/reply/{id}', 'Back\UnblockRequestController@reply')->name('back.unblock_request.reply');
        Route::post('unblock-requests/fine/{id}', 'Back\UnblockRequestController@imposeFine')->name('back.unblock_request.fine');
        Route::post('unblock-requests/unblock/{id}', 'Back\UnblockRequestController@unblock')->name('back.unblock_request.unblock');
        Route::post('unblock-requests/reblock/{id}', 'Back\UnblockRequestController@reblock')->name('back.unblock_request.reblock');
        Route::get('unblock-requests/delete/{id}', 'Back\UnblockRequestController@delete')->name('back.unblock_request.delete');

        //------------ FINE APPROVALS ------------
        Route::get('fine-approvals', 'Back\FineApprovalController@index')->name('back.fine_approval.index');
        Route::post('fine-approvals/approve/{id}', 'Back\FineApprovalController@approve')->name('back.fine_approval.approve');
        Route::post('fine-approvals/reject/{id}', 'Back\FineApprovalController@reject')->name('back.fine_approval.reject');

        //------------ STORES RATE & SETTING ------------
        Route::get('store-settings', 'Back\StoreSettingController@index')->name('back.store_setting.index');
        Route::post('store-settings/update', 'Back\StoreSettingController@update')->name('back.store_setting.update');
        Route::post('store-settings/account/store', 'Back\StoreSettingController@storeAccount')->name('back.store_setting.account.store');
        ...
        Route::get('vendor-products/{id}', 'Back\VendorProductController@show')->name('back.vendor_product.show');
        Route::post('vendor-products/{id}/approve', 'Back\VendorProductController@approve')->name('back.vendor_product.approve');
        Route::post('vendor-products/{id}/reject', 'Back\VendorProductController@reject')->name('back.vendor_product.reject');
        Route::delete('vendor-products/{id}', 'Back\VendorProductController@destroy')->name('back.vendor_product.destroy');

        //------------ DEPOSIT REQUESTS (VENDOR WALLET TOPUPS) ------------
        Route::get('deposit-requests', 'Back\DepositRequestController@index')->name('back.deposit_request.index');
        Route::get('deposit-requests/approve/{id}', 'Back\DepositRequestController@approve')->name('back.deposit_request.approve');
        Route::post('deposit-requests/reject/{id}', 'Back\DepositRequestController@reject')->name('back.deposit_request.reject');

        // Aliases
        Route::get('vendor-products-alias', 'Back\VendorProductController@index')->name('admin.vendor_product.index');
        Route::get('vendor-products-alias/{id}', 'Back\VendorProductController@show')->name('admin.vendor_product.show');
        Route::post('vendor-products-alias/{id}/approve', 'Back\VendorProductController@approve')->name('admin.vendor_product.approve');
        Route::post('vendor-products-alias/{id}/reject', 'Back\VendorProductController@reject')->name('admin.vendor_product.reject');
        Route::delete('vendor-products-alias/{id}', 'Back\VendorProductController@destroy')->name('admin.vendor_product.destroy');
        ...
        Route::group(['middleware' => 'permissions:Manage Orders'], function () {
            ...
            Route::post('/order/send-customer-email/{id}', 'Back\OrderController@sendCustomerEmail')->name('back.order.send.email');
        });
        ...
        Route::group(['middleware' => 'permissions:Manage Categories'], function () {
            //------------ CATEGORY ------------
            Route::post('category/limit/update', 'Back\CategoryController@updateLimit')->name('back.category.limit.update');
        ...
        Route::group(['middleware' => 'permissions:Ecommerce'], function () {
            ...
            Route::post('/setting/payment/custom/store', 'Back\PaymentSettingController@customStore')->name('back.setting.payment.custom.store');
            Route::post('/setting/payment/custom/update/{id}', 'Back\PaymentSettingController@customUpdate')->name('back.setting.payment.custom.update');
            Route::delete('/setting/payment/custom/delete/{id}', 'Back\PaymentSettingController@customDelete')->name('back.setting.payment.custom.delete');
        });
        ...
        //------------ CUSTOMER LIVE CHATS / ORIVO MESSAGES ------------
        Route::get('messages', 'Back\ChatController@index')->name('back.message.index');
        Route::post('messages/send', 'Back\ChatController@send')->name('back.message.send');
        Route::get('messages/fetch/{id}', 'Back\ChatController@fetch')->name('back.message.fetch');
        Route::get('messages/delete/{id}', 'Back\ChatController@delete')->name('back.message.delete');

        //------------ BUYER & SELLER CHATS (MODERATION & PARTICIPATION) ------------
        Route::get('buyer-seller-chats', 'Back\BuyerSellerChatController@index')->name('back.buyer_seller_chat.index');
        Route::post('buyer-seller-chats/send', 'Back\BuyerSellerChatController@send')->name('back.buyer_seller_chat.send');
        Route::get('buyer-seller-chats/fetch/{id}', 'Back\BuyerSellerChatController@fetch')->name('back.buyer_seller_chat.fetch');
        Route::get('buyer-seller-chats/delete/{id}', 'Back\BuyerSellerChatController@delete')->name('back.buyer_seller_chat.delete');
        ...
        Route::group(['middleware' => 'permissions:Transactions'], function () {
            ...
            Route::get('/transaction/{id}', 'Back\TranactionController@show')->name('back.transaction.show');
        ...
        Route::group(['middleware' => 'permissions:Manage Site'], function () {
            ...
            // ----------- WHATSAPP SETTING ---------------//
            Route::get('/setting/whatsapp', 'Back\WhatsAppSettingController@index')->name('back.setting.whatsapp');
            Route::post('/setting/whatsapp/update', 'Back\WhatsAppSettingController@update')->name('back.whatsapp.update');
            Route::post('/setting/whatsapp/test', 'Back\WhatsAppSettingController@test')->name('back.whatsapp.test');
            // ----------- WHATSAPP SETTING ---------------//
        ...
        //------------ CHECKOUT MESSAGE ------------
        Route::get('/checkout-message', 'Back\CheckoutMessageController@index')->name('back.checkout.message');
...
Route::group(['middleware' => 'maintainance'], function () {
    Route::group(['middleware' => 'localize'], function () {
        ...
        Route::prefix('user')->group(function () {
            ...
            //------------ OPEN SHOP / STORE APPLICATION ------------
            Route::get('/open-shop', 'User\StoreApplyController@apply')->name('user.store.apply');
            Route::post('/open-shop/submit', 'User\StoreApplyController@submit')->name('user.store.submit');

            //------------ CHATS / MESSAGES ------------
            Route::get('/messages', 'User\ChatController@index')->name('user.message.index');
            Route::post('/messages/send', 'User\ChatController@send')->name('user.message.send');
            Route::get('/messages/fetch/{id}', 'User\ChatController@fetch')->name('user.message.fetch');
            Route::get('/messages/delete/{id}', 'User\ChatController@delete')->name('user.message.delete');
        });
        ...
        Route::prefix('seller')->group(function () {
            ...
            Route::get('impersonate/leave', 'Seller\DashboardController@leaveImpersonation')->name('seller.impersonate.leave');

            //------------ BLOCKED PAGE & UNBLOCK REQUEST & FINE PAYMENT ------------
            Route::get('blocked', 'Seller\DashboardController@blocked')->name('seller.blocked');
            Route::post('unblock-request/submit', 'Seller\DashboardController@submitUnblockRequest')->name('seller.unblock.submit');
            Route::post('fine/submit', 'Seller\DashboardController@submitFinePayment')->name('seller.fine.submit');
            Route::post('fine/pay-wallet', 'Seller\DashboardController@payFineFromWallet')->name('seller.fine.pay_wallet');

            //------------ DASHBOARD ------------
            Route::get('/', 'Seller\DashboardController@index')->name('seller.dashboard');
            Route::get('/dashboard', 'Seller\DashboardController@index')->name('seller.dashboard.index');

            //------------ CUSTOMER MESSAGES / LIVE CHAT ------------
            Route::get('messages', 'Seller\ChatController@index')->name('seller.message.index');
            Route::post('messages/send', 'Seller\ChatController@send')->name('seller.message.send');
            Route::get('messages/fetch/{id}', 'Seller\ChatController@fetch')->name('seller.message.fetch');
            Route::get('messages/delete/{id}', 'Seller\ChatController@delete')->name('seller.message.delete');
            ...
            Route::get('item/status/{id}/{status}', 'Seller\ItemController@status')->name('seller.item.status');
            Route::get('get/subcategory', 'Seller\ItemController@getsubCategory')->name('seller.get.subcategory');
            Route::get('get/childcategory', 'Seller\ItemController@getChildCategory')->name('seller.get.childcategory');
            Route::get('stock/out/product', 'Seller\ItemController@stockOut')->name('seller.item.stock.out');
            Route::resource('item', 'Seller\ItemController', ['as' => 'seller', 'except' => 'show']);
            Route::get('item/galleries/{item}', 'Seller\ItemController@galleries')->name('seller.item.gallery');
            ...
            //------------ ORDERS ------------
            Route::get('orders', 'Seller\OrderController@index')->name('seller.order.index');
            Route::get('order', 'Seller\OrderController@index');
            Route::get('order/invoice/{id}', 'Seller\OrderController@invoice')->name('seller.order.invoice');
            Route::get('order/print/{id}', 'Seller\OrderController@printOrder')->name('seller.order.print');
            Route::get('order/status/{id}/{field}/{value}', 'Seller\OrderController@status')->name('seller.order.status');
            ...
            Route::get('transaction/{id}', 'Seller\TransactionController@show')->name('seller.transaction.show');

            //------------ CATEGORIES ------------
            Route::post('category/quick-store', 'Seller\CategoryController@quickStore')->name('seller.category.quick.store');
            Route::get('category/status/{id}/{status}', 'Seller\CategoryController@status')->name('seller.category.status');
            Route::resource('category', 'Seller\CategoryController', ['as' => 'seller', 'except' => ['show']]);

            //------------ SUB CATEGORIES ------------
            Route::post('subcategory/quick-store', 'Seller\SubCategoryController@quickStore')->name('seller.subcategory.quick.store');
            Route::get('subcategory/status/{id}/{status}', 'Seller\SubCategoryController@status')->name('seller.subcategory.status');
            Route::resource('subcategory', 'Seller\SubCategoryController', ['as' => 'seller', 'except' => ['show']]);

            //------------ CHILD CATEGORIES ------------
            Route::post('childcategory/quick-store', 'Seller\ChieldCategoryController@quickStore')->name('seller.childcategory.quick.store');
            Route::get('childcategory/status/{id}/{status}', 'Seller\ChieldCategoryController@status')->name('seller.childcategory.status');
            Route::resource('childcategory', 'Seller\ChieldCategoryController', ['as' => 'seller', 'except' => ['show']]);

            //------------ BRANDS ------------
            Route::post('brand/quick-store', 'Seller\BrandController@quickStore')->name('seller.brand.quick.store');
            Route::get('brand/status/{id}/{status}/{type}', 'Seller\BrandController@status')->name('seller.brand.status');
            Route::resource('brand', 'Seller\BrandController', ['as' => 'seller', 'except' => ['show']]);

            //------------ STORE PROFILE / SETTINGS ------------
            Route::get('profile', 'Seller\AccountController@profile')->name('seller.profile');
            Route::post('profile/update', 'Seller\AccountController@updateProfile')->name('seller.profile.update');
        });
        // ************************************ SELLER (VENDOR) PANEL ENDS **********************************************
        ...
        //------------ LIVE CHAT / SELLER INQUIRY ------------
        Route::post('/chat/init', 'Front\ChatController@init')->name('front.chat.init');
        Route::post('/chat/send', 'Front\ChatController@send')->name('front.chat.send');
        Route::get('/chat/fetch/{id}', 'Front\ChatController@fetch')->name('front.chat.fetch');
        Route::delete('/chat/delete/{id}', 'Front\ChatController@delete')->name('front.chat.delete');
        ...
        Route::get('/cart/destroy/{id?}', 'Front\CartController@destroy')->name('front.cart.destroy')->where('id', '.*');
        ...
        Route::post('/checkout/check-transaction-id', 'Front\CheckoutController@checkTransactionId')->name('front.checkout.check.txn');
