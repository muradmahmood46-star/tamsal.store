...
class CartRepository
{
    ...
    public function store($request)
    {
        ...
        // Check variant combination stock if variants exist
        if (!empty($item->item_variants)) {
            $variants = json_decode($item->item_variants, true);
            if (is_array($variants) && count($variants) > 0) {
                $targetOptions = $attribute['option_name'] ?? [];
                foreach ($variants as $v) {
                    ...
                    if ($matchColor && $matchSize) {
                        ...
                        $existingQty = 0;
                        if (Session::has('cart')) {
                            $curCart = Session::get('cart');
                            if (isset($curCart[$item->id . '-' . $cart_item_key]) && $qty_check == 0) {
                                $existingQty = (int)$curCart[$item->id . '-' . $cart_item_key]['qty'];
                            }
                        }
                        if (($existingQty + (int)$qty) > $vStock) {
                            return ['message' => __('Selected Variant is Out Of Stock'), 'status' => 'outStock'];
                        }
                        break;
                    }
                }
