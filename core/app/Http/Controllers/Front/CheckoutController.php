...
class CheckoutController extends Controller
{
    ...
    public function checkoutPage()
    {
        ...
        foreach ($cart as $key => $item) {
            $total += ($item['main_price'] + ($item['attribute_price'] ?? 0)) * $item['qty'];
            $cart_total = $total;
        ...
        $data['tax'] = $total_tax;
        $data['payments'] = PriceHelper::getCheckoutPaymentGateways($cart);
    ...
    public function payment()
    {
        ...
        if (PriceHelper::CheckDigital() == true) {
            $actual_delivery_fee = PriceHelper::getDeliveryFee($cart, $cart_total, false);
            $delivery_fee = $actual_delivery_fee;
            $shipping_price = $delivery_fee;
            $shipping = (object)[
                'id' => 1,
                'title' => $delivery_fee > 0 ? __('Delivery Charges') : __('Free Delivery'),
                'price' => $delivery_fee
            ];
        }
        ...
        $data['actual_delivery_fee'] = $actual_delivery_fee;
        $data['tax'] = $total_tax;
        $data['payments'] = PriceHelper::getCheckoutPaymentGateways($cart);
    ...
    public function checkTransactionId(Request $request)
    {
        ...
        return response()->json([
            'status' => true,
            'exists' => false,
            'message' => __('Transaction ID is valid & available.')
        ]);
    }
    ...
    public function checkout(PaymentRequest $request)
    {
        ...
        switch ($input['payment_method']) {
            ...
            case 'Stripe':
                $checkout = true;
                $payment = $this->ManualSubmit($input, 'Easypaisa');
                break;

            case 'Authorize.Net':
                $checkout = true;
                $payment = $this->ManualSubmit($input, 'JazzCash');
                break;
            ...
            case 'Bank':
                $checkout = true;
                $payment = $this->ManualSubmit($input, 'Bank Transfer');
                break;
            ...
            default:
                if (\App\Models\PaymentSetting::where('name', $input['payment_method'])->orWhere('unique_keyword', $input['payment_method'])->exists()) {
                    $checkout = true;
                    $payment = $this->ManualSubmit($input, $input['payment_method']);
                }
                break;
        }
    ...
    public function stateSetUp(Request $request)
    {
        ...
        $delivery_fee = PriceHelper::getDeliveryFee($cart, $cart_total);
